...
function ajaxRequest(aElement, aUrl, objectType){
    ...
    req.onreadystatechange = function(){
        ...
        for(var x = 0; x < results.length; x ++){
                         ...
                         } else if(object_type == 'drug'){
                             li.innerHTML = results[x].name;
                             li.setAttribute('tstvalue', results[x].drug_id)
                             li.setAttribute('value', results[x].name)
                             li.setAttribute('id', results[x].rec_id);
                        }else if(object_type == 'patient_identifiers'){
                             li.innerHTML = results[x].name;
                             li.setAttribute('tstvalue', results[x].patient_identifier_type_id)
                             li.setAttribute('value', results[x].patient_identifier_type_id)
                             li.setAttribute('id', results[x].patient_identifier_type_id);
                        }else if(object_type == 'labs'){
                            console.log(results[x]);
                             li.innerHTML = results[x];
                             li.setAttribute('tstvalue', results[x]);
                             li.setAttribute('value', results[x]);
                        }
                        else{
                            li.innerHTML = results[x];
                            li.setAttribute('tstValue', results[x]);
