...
function validationHandler (field) {
    var handler = field.getAttribute('handler')
    if (handler !== null) {
        var message = field.getAttribute('handlerMessage') || 'Handler validation failed.'
	    if (!eval(handler + '(' + field.value + ')')) return message   
    }
	return ''
}
...
TTInput.prototype = {
    ...
    validate: function() {
        ...
        if (this.value.length > 0 || !this.element.getAttribute('optional')){
            ...
            // validates using handler
            errorMsg = this.validateHandler();
            if (errorMsg.length > 0) return errorMsg;
    ...
    validateHandler: function () {
        return validationHandler(this.element)
    },
