...
var tstInternalCurrentDate = (padZeros((new Date().getDate()) + "-" + padZeros((new Date().getMonth() + 1),2) + "-" + 
...
function loadSelectOptions(selectOptions, options, dualViewOptions) {    
    ...
    for(var j=0;j<selectOptionCount;j++){
        ...
        optionsList += ">" + (tstFormElements[tstCurrentPage].getAttribute("multiple") ? 
            ...
            "' src='/public/touchscreentoolkit/lib/images/unticked.jpg' alt='[ ]' />" +
            ...
            if(optionIncludedInValue(unescape(options[i].innerHTML), val_arr)){
                ...
                if(tstFormElements[tstCurrentPage].getAttribute("multiple")){
                    ...
                    if(id){
                        ...
                        __$("img" + id[1]).src = "/public/touchscreentoolkit/lib/images/ticked.jpg";
                    }            
                }
                ...
                if(tstFormElements[tstCurrentPage].getAttribute("multiple")){
                    ...
                    if(id){
                        ...
                        __$("img" + id[1]).src = "/public/touchscreentoolkit/lib/images/unticked.jpg";
                    }            
                }
...
function ajaxRequest(aElement, aUrl, objectType){
    ...
    url = new URL(url);
    var program_id = url.searchParams.get("program_id");

    if(objectType == 'states' && parseInt(program_id) != 1) {                  
      aUrl = '/programs/' + program_id + '/workflows';
    }
    ...
    req.onreadystatechange = function(){
        ...
        if(object_type == 'states'){
            var program = results[0]
            results = results[0].states;
        }
        ...
        for(var x = 0; x < results.length; x ++){
						 ...
						 if(object_type == 'region'){
                            ...
                            if (program.program_id === 1) {
                                var currentStateId = results[x].program_workflow_state_id 
                                var artTerminalStateIds = [2, 3, 6]
                                var currentStateIsTerminal = results[x].terminal
                                var stateIsAnArtTerminalState = artTerminalStateIds.indexOf(currentStateId) >= 0

                                if (currentStateIsTerminal && stateIsAnArtTerminalState) {
                                    li.innerHTML = results[x].concept.concept_names[0].name;
                                    li.setAttribute('tstValue', currentStateId);
                                    li.setAttribute('terminal', currentStateIsTerminal);
                                    li.setAttribute('id', x);
                                }
                            } else {
                                li.innerHTML = results[x].concept.concept_names[0].name;
                                li.setAttribute('tstValue', results[x].program_workflow_state_id );
                                li.setAttribute('terminal', results[x].terminal);
                                li.setAttribute('id', x);
                            }
                            ...
                            li.setAttribute('id', x)
                        } else if(object_type == 'location'){
                            li.innerHTML = results[x].name;
                            li.setAttribute('tstvalue', results[x].location_id)
                            li.setAttribute('id', results[x].location_id);
                        }else if(object_type == 'map_lab_panel'){
                            li.innerHTML = results[x].name;
                            li.setAttribute('tstvalue', results[x].rec_id)
                            li.setAttribute('value', results[x].name)
                            li.setAttribute('id', results[x].rec_id);
                        }else{
						 ...
						 li.setAttribute('onclick',"null; updateTouchscreenInputForSelect(this); checkRequireNextClick();")
						 ol.appendChild(li);
				}
...
function showStatus(){
    if(!__$("popupBox")){
        ...
        innerPop.id = "innerPop";
        innerPop.style.width = "100%";
        innerPop.style.height = "100%";
        innerPop.style.top = "0";
        innerPop.style.position = "absolute";
        innerPop.style.zIndex = "1000";
        ...
        innerPop.appendChild(popupBox);
        __$("content").appendChild(innerPop);
    }
